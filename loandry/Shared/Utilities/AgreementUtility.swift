...
struct AgreementUtility {
    static func generate(for loan: Loan) -> String {
        ...
        // Calculate a concrete first payment date if one wasn't explicitly set
        let effectiveFirstPaymentDate: Date
        if let explicitDate = loan.first_payment_date {
            effectiveFirstPaymentDate = explicitDate
        } else {
            // Default logic: 1 month after creation for monthly, otherwise same as maturity (lump sum)
            let anchorDate = loan.created_at ?? Date()
            if loan.repayment_schedule.lowercased().contains("month") {
                effectiveFirstPaymentDate = Calendar.current.date(byAdding: .month, value: 1, to: anchorDate) ?? loan.maturity_date
            } else {
                effectiveFirstPaymentDate = loan.maturity_date
            }
        }
        
        let firstPaymentString = effectiveFirstPaymentDate.formatted(date: .long, time: .omitted)
        ...
        The first payment is due on \(firstPaymentString).
    ...
    static func generateRelease(for loan: Loan) -> String {
        ...
        [ ELECTRONICALLY GENERATED ON LOANDRY ]
        """
    }
}
